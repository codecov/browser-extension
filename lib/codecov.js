...
window.Codecov = (function() {
  ...
  Codecov.prototype._get = function(endpoint) {
    ...
    if (endpoint === 'https://codecov.io') {
      ...
      url = (endpoint + "/api/" + this.service + "/" + this.slug + "?ref=" + this.ref) + (this.base ? "&base=" + this.base : "") + "?src=extension";
    }
    return $.ajax({
      url: url,
      type: 'get',
...
window.Github = (function(superClass) {
  ...
  Github.prototype.overlay = function(res) {
    var _td, compare, plus, ref, ref1, ref2, ref3, ref4, report, self, split_view, total, total_hits, total_lines;
    this.log('::overlay');
    self = this;
    ...
    if (this.page === 'tree') {
      ...
      $('.repository-content .file').each(function() {
        var button, diff, file, file_data, file_lines, fp, hits, lines, ref4, ref5, ref6, ref7, ref8;
        file = $(this);
        fp = self.file || file.find('.file-info>span[title]').attr('title');
        if (fp) {
          file_data = report.files[fp];
          if ((file_data == null) || file_data.ignored) {
            return;
          }
          if (file.find('.file-actions > .btn-group').length === 0) {
            file.find('.file-actions a:first').wrap('<div class="btn-group"></div>');
          ...
          if (file_data != null) {
            total = ((ref4 = file_data.t) != null ? ref4.c : void 0) != null ? file_data.t.c : file_data.coverage;
            button = file.find('.btn.codecov').attr('aria-label', 'Toggle Codecov (c), alt+click to open in Codecov').attr('data-codecov-url', (self.settings.urls[self.urlid] + "/" + self.service + "/" + self.slug + "/") + (((ref6 = file_data.t) != null ? ref6.c : void 0) != null ? "src/" + self.ref + "/" + fp : fp + "?ref=" + self.ref)).text((self.format(total)) + "%").removeClass('disabled').unbind().click((ref5 = self.page) === 'blob' || ref5 === 'blame' ? self.toggle_coverage : self.toggle_diff);
            hits = 0;
            lines = 0;
            ...
            file.find('tr').each(function() {
              var cov, ref7, td;
              td = $(_td, this);
              cov = self.color(file_lines[td.attr('data-line-number') || ((ref7 = td.attr('id')) != null ? ref7.slice(1) : void 0)]);
              if (cov) {
                if (split_view) {
