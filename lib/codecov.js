...
window.Codecov = (function() {
  ...
  Codecov.prototype._get = function(endpoint) {
    ...
    self = this;
    return $.ajax({
      url: endpoint + "/api/" + this.service + "/" + this.slug + "/commits/" + this.ref + this.base,
      type: 'get',
      dataType: 'json',
  ...
  Codecov.prototype.color = function(ln) {
    ...
    if (ln.c === 0) {
      ...
      return "partial";
    } else if (indexOf.call(ln.c.toString(), '/') >= 0) {
      v = ln.c.split('/');
      if (v[0] === '0') {
        return "missed";
      } else if (v[0] === v[1]) {
        ...
        if (fp) {
          ...
          if (file_data) {
            ...
            file.find('tr').each(function() {
              var cov, ref4, td;
              td = $(_td, this);
              cov = self.color(file_data['l'][td.attr('data-line-number') || ((ref4 = td.attr('id')) != null ? ref4.slice(1) : void 0)]['c']);
              if (cov) {
                if (split_view) {
