...
window.Github = (function(superClass) {
  ...
  Github.prototype.get_ref = function(href) {
    ...
    if ((ref = this.page) === 'releases' || ref === 'tags') {
      ...
      return $('.file-wrap a.js-directory-link:first').attr('href').split('/')[4];
    } else {
      return false;
    }
  };
  ...
  Github.prototype.overlay = function(res) {
    ...
    if (this.page === 'tree') {
      $('.commit-meta').prepend("<a href=\"" + this.settings.urls[this.urlid] + "/github/" + this.slug + "?ref=" + this.ref + "\" class=\"sha-block codecov tooltipped tooltipped-n\" aria-label=\"Overall coverage\">" + (Math.floor(res['report']['coverage'])) + "%</a>");
...
window.Bitbucket = (function(superClass) {
  ...
  Bitbucket.prototype.overlay = function(res) {
    ...
    $('section.bb-udiff').each(function() {
      ...
      if (!(coverage != null ? coverage.ignored : void 0)) {
        ...
        if (coverage) {
          ...
          self.log($('.udiff-line.common, .udiff-line.addition', this).find('a.line-numbers'));
    ...
    return $("#editor-container").each(function() {
      var _, button, cov, coverage, file, fp, i, len, ln, ref, ref1, ref2, results;
      file = $(this);
      fp = file.attr('data-path');
      coverage = res['report']['files'][fp] || self.find_best_fit_path(fp, res['report']['files']);
      if (!(coverage != null ? coverage.ignored : void 0)) {
        if (coverage) {
          ...
          for (ln in ref) {
            cov = ref[ln];
            $("a[name=cl-" + ln + "]", file).addClass("codecov codecov-" + (self.color(cov)));
          }
  ...
  Bitbucket.prototype.error = function(status, reason) {
    if (status === 401) {
      ...
      return $('.aui-button.codecov').text("Umbrella required").addClass('aui-button-primary').attr('title', 'Umbrella account require to view private repo reports').click(function() {
        return window.location = "https://codecov.io/umbrella";
      });
    } else if (status === 404) {
