...
window.BitbucketServer = (function(superClass) {
  ...
  function BitbucketServer() {
    this.pr_view = bind(this.pr_view, this);
  ...
  BitbucketServer.prototype.ref = '';

  BitbucketServer.prototype.href = '';

  BitbucketServer.prototype.sub_page = '';

  BitbucketServer.prototype.timeout = 1500;

  BitbucketServer.prototype.time_interval = 0;

  BitbucketServer.prototype.service = 'bbs';
  ...
  BitbucketServer.prototype.bbs_rest_request = function(rest_path, callback) {
    var url;
    url = window.location.protocol + "//" + window.location.host + "/rest/api/latest/" + rest_path;
    this.log('::bbs_rest_request', url);
    return $.ajax({
      url: url,
      success: function(res) {
        return callback(res);
      ...
      error: function(xhr, type, reason) {
        return self.error(xhr.status, reason);
      }
    });
  };

  BitbucketServer.prototype.refresh_href = function() {
    var href;
    this.log('::refresh_href');
    href = (this.settings.debug_url || window.location.href).split('/');
    if (href !== this.href) {
      this.base = '';
      this.ref = '';
      this.href = href;
  ...
  BitbucketServer.prototype.get_ref = function(href) {
    var branch_str, commit_href, json, ref, self;
    this.log('::get_ref');
    self = this;
    switch (this.page) {
      case 'browse':
        if ($('.commitid').length !== 0) {
          this.ref = $('.commitid').attr('data-commitid');
      ...
      case 'commits':
        ...
        if (href[7] === 'commits' && typeof href[8] === 'undefined') {
          ...
          if (commit_href[1] !== '?merges=include') {
            ...
            this.ref = branch_str[branch_str.length - 1];
          }
        } else {
          if (href[8].indexOf('#') !== -1) {
            this.ref = href[8].split('#')[0];
          } else {
            this.ref = href[8];
          }
        }
        break;
      case 'pull-requests':
        if (this.sub_page === 'commits' && this.href[this.href.length - 1] === 'commits') {
          if ($('.branch_name').length > 0) {
            this.ref = $($('.branch_name')[0]).text();
          }
        }
    }
    return false;
  ...
  BitbucketServer.prototype.update_view = function() {
    var href_changed, rest_path, self;
    this.log('::update_view');
    self = this;
    href_changed = this.refresh_href();
    if (href_changed) {
      if (this.page === 'pull-requests' && this.href[this.href.length - 1] !== 'commits') {
        rest_path = (this.href.slice(3, 9).join('/')) + "/commits";
        this.bbs_rest_request(rest_path, function(response) {
          var commit_obj;
          if (self.sub_page === 'diff') {
            ...
            commit_obj = $.grep(response.values, function(e) {
              return e.id === self.ref;
            });
  ...
  BitbucketServer.prototype.send_signal = function() {
    ...
    return this.time_interval = setTimeout(this.update_view, this.timeout);
  };

  BitbucketServer.prototype.register_bind_id = function(bind_id) {
    var self;
    self = this;
    return $(bind_id).bind("DOMSubtreeModified", function() {
  ...
  BitbucketServer.prototype.parse_href = function() {
    this.log('::parse_href');
    this.slug = this.href[4] + "/" + this.href[6];
    this.page = this.href[7].split('?')[0];
    if (this.page === 'pull-requests' && this.href.length >= 9) {
      return this.sub_page = this.href[9];
    }
  ...
  BitbucketServer.prototype.prepare = function() {
    ...
    switch (this.page) {
      case 'browse':
        if ($('.file-content').length !== 0) {
          $('.file-toolbar > .secondary').prepend('<div class="aui-buttons"><button class="aui-button codecov-button" title="Toggle coverage from codecov"> Coverage </button></div>');
        } else if ($('.filebrowser-content').length !== 0) {
          $('.aui-toolbar2-secondary > .aui-buttons').append('<button class="aui-button codecov-button" title="Toggle coverage from codecov"> Coverage </button>');
        }
        break;
      case 'commits':
        if ($('.commits-table').length !== 0) {
          $('.aui-toolbar2-secondary').append('<div class="aui-buttons"><button class="aui-button codecov-button" title="Toggle coverage from codecov"> Coverage </button></div>');
        } else if ($('.file-content').length !== 0) {
          setTimeout(function() {
            $('#commit-file-content > .file-toolbar > .secondary').prepend('<button class="aui-button codecov-button" title="Toggle coverage from codecov"> Coverage </button>');
            return $('.codecov-button').on('click', function() {
    ...
    $('.codecov-button').on('click', function() {
      return self.update_view();
    });
    return true;
  };

  BitbucketServer.prototype.browse_view = function(res) {
    var button, cov, filename, key, line, ref1, ref2, ref3, ref4, report, results1, self, value;
    this.log('::browse_view', res);
    self = this;
    ...
    if ($('#browse-table tr').length > 0) {
      $('#browse-table tr').each(function() {
        var cov, ref3, ref4;
        if ($(this).hasClass('folder') || $(this).children('.codecov-on').length !== 0) {
          return true;
        }
        filename += "/" + ($(this).find('td.item-name').children('a').text());
        cov = report != null ? (ref3 = report.files) != null ? (ref4 = ref3[filename]) != null ? ref4.t.c : void 0 : void 0 : void 0;
        if (cov != null) {
  ...
  BitbucketServer.prototype.commits_view = function(res) {
    var base_url, button, c, commits, cov, coverage_url, filename, i, key, len, line, ref1, ref2, ref3, ref4, ref5, report, results1, self, value;
    this.log('::commits_view', res);
    self = this;
    report = (res != null ? (ref1 = res.commit) != null ? ref1.report : void 0 : void 0) || (res != null ? (ref2 = res.head) != null ? ref2.report : void 0 : void 0);
    filename = '';
    if ($('#commits-table').length === 0) {
      ...
      if (cov != null) {
        ...
        for (key in ref4) {
          ...
          results1.push(line.children('.CodeMirror-gutter-wrapper').find('.line-number').addClass("codecov codecov-" + (self.color(value))));
        }
        return results1;
      }
    } else {
      ...
      return $('#commits-table tr').each(function() {
        ...
        if ((commit_data != null ? (ref6 = commit_data.totals) != null ? ref6.c : void 0 : void 0) != null) {
          return commit.find('.message').append("<a style=\"float:right;\" class=\"codecov-on\" title=\"Codecov Coverage\" href=\"" + base_url + "/" + commit_data.commitid + "\">" + (self.format(commit_data.totals.c)) + "%</Aa>");
        }
      });
    }
  };

  BitbucketServer.prototype.pr_view = function(res) {
    var base_url, c, commits, cov, filename, i, j, key, len, len1, line, pull, pulls, ref1, ref2, ref3, ref4, report, results1, results2, self, value;
    this.log('::pr_view', res);
    self = this;
    filename = '';
    if (this.sub_page) {
      switch (this.sub_page) {
        ...
        case 'diff':
          ...
          break;
        case 'commits':
          if (this.href[this.href.length - 1] === 'commits') {
            commits = {};
            ref2 = res.commits;
            ...
            base_url = this.settings.urls[this.urlid] + "/" + this.service + "/" + this.slug + "/commit";
            return $('#pull-request-commits-table tr').each(function() {
              var commit, commit_data, commitid, ref3;
              commit = $(this);
              if (commit.find('.message').children('.codecov-on').length > 0) {
                return true;
              }
              commitid = commit.attr('data-commitid');
              commit_data = commits[commitid];
              if ((commit_data != null ? (ref3 = commit_data.totals) != null ? ref3.c : void 0 : void 0) != null) {
                return commit.find('.message').append("<a style=\"float:right;\" class=\"codecov-on\" title=\"Codecov Coverage\" href=\"" + base_url + "/" + commit_data.commitid + "\">" + (self.format(commit_data.totals.c)) + "%</Aa>");
              }
            });
          } else {
            ...
            for (key in ref3) {
              value = ref3[key];
              line = $('.side-by-side-diff-editor-to').find('.CodeMirror-code .line:nth-child(' + key + ')');
              results2.push(line.children('.CodeMirror-gutter-wrapper').find('.line-number').addClass("codecov codecov-" + (self.color(value))));
            }
            return results2;
          }
      }
      ...
      if ($('.pull-requests-table').length > 0) {
        ...
        return $('.pull-requests-table tr').each(function(index, element) {
          var pullid, summary;
          summary = $(this.children('.summary'));
          ...
          if (line.children('.codecov-on').length > 0) {
            ...
            return line.append("<a class=\"codecov-on\" title=\"Codecov Coverage\" href=\"" + base_url + "/" + pullid + "\">" + (self.format(pulls.pullid.totals.c)) + "%</Aa>");
          }
        });
      }
    }
  ...
  BitbucketServer.prototype.overlay = function(res) {
    this.log('::overlay');
    if (this.page === 'commits') {
      return this.commits_view(res);
    } else if (this.page === 'browse') {
      return this.browse_view(res);
    } else if (this.page === 'pull-requests') {
      return this.pr_view(res);
    }
  };
  ...
  BitbucketServer.prototype.error = function(status, reason) {
    if (status === 401) {
      return $('.aui-button.codecov').text("Please login at Codecov").addClass('aui-button-primary').attr('title', 'Login to view coverage by Codecov').click(function() {
