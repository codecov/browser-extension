...
window.Github = (function(superClass) {
  ...
  Github.prototype.overlay = function(res) {
    var compare, coverage, plus, ref, ref1, replacement, self, total_hits, total_lines;
    this.log('::overlay');
    self = this;
    $('.codecov-removable').remove();
    if (this.page === 'tree') {
      replacement = "/" + self.slug + "/blob/" + ($('.file-navigation .repo-root a:first').attr('data-branch')) + "/";
      $('.commit-tease .right').append("<a href=\"" + this.settings.urls[this.urlid] + "/github/" + this.slug + "?ref=" + this.ref + "\" class=\"sha-block codecov codecov-removable tooltipped tooltipped-n\" aria-label=\"Overall coverage\">" + (res['report']['coverage'].toFixed(2)) + "%</a>");
      ...
      total_lines = 0;
      $('.repository-content .file').each(function() {
        var _td, button, coverage_precent, diff, file, fp, hits, lines, ref1, ref2, ref3;
        file = $(this);
          ...
          if (!(coverage != null ? coverage.ignored : void 0)) {
            ...
            if (coverage) {
              ...
              if ((ref2 = self.page) === 'commit' || ref2 === 'compare' || ref2 === 'pull') {
                ...
                $('#toc a[href="#' + file.prev().attr('name') + '"]').parent('span.diffstat').prepend("<span class=\"codecov codecov-removable\">" + coverage_precent + "% <strong>(" + diff + "%)</strong></span>");
              }
      ...
      if ((ref1 = self.page) === 'commit' || ref1 === 'compare' || ref1 === 'pull') {
        return $('.toc-diff-stats .codecov').append(" (Diff <strong>" + (self.ratio(total_hits, total_lines)) + "%</strong>)</span>");
      }
    }
  };
