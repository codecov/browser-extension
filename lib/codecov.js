...
window.Github = (function(superClass) {
  ...
  Github.prototype.overlay = function(res) {
    var branch, compare, coverage, plus, ref, self;
    self = this;
    if (this.page === 'tree') {
      branch = $('.file-navigation .repo-root a:first').attr('data-branch');
      $('.commit-meta').remove('.codecov').prepend("<a href=\"" + this.settings.urls[this.urlid] + "/github/" + this.slug + "?ref=" + this.ref + "\" class=\"sha-block codecov tooltipped tooltipped-n\" aria-label=\"Overall coverage\">" + res['report']['coverage'] + "%</a>");
      return $('.file-wrap tr:not(.warning):not(.up-tree)').each(function() {
        var coverage, filepath, ref, ref1, ref2;
        filepath = (ref = $('td.content a', this).attr('href')) != null ? ref.replace("/" + self.slug + "/blob/" + branch + "/", '') : void 0;
        if (filepath) {
          ...
          if (!(coverage != null ? coverage.ignored : void 0)) {
            if (coverage >= 0) {
              return $('td:last', this).remove('.codecov').append("<span class=\"sha codecov tooltipped tooltipped-n\" aria-label=\"Coverage\">" + coverage + "%</span>");
            }
          }
        }
      });
