...
window.Codecov = (function() {
  ...
  Codecov.prototype._get = function(endpoint) {
    ...
    self = this;
    return $.ajax({
      url: endpoint + "/" + this.service + "/" + this.slug + "?ref=" + this.ref + this.base,
      type: 'get',
      dataType: 'json',
      headers: {
      ...
      error: function(xhr, type, reason) {
        ...
        if (self.settings.urls.length > self.urlid + 1) {
          self._get(self.settings.urls[self.urlid += 1]);
        }
...
window.Github = (function(superClass) {
  ...
  Github.prototype.overlay = function(res) {
    var compare, coverage, plus, ref, self;
    if (this.page === 'tree') {
      $('.commit-meta').remove('.codecov').prepend("<a href=\"" + this.settings.urls[this.urlid] + "/github/" + this.slug + "?ref=" + this.ref + "\" class=\"sha-block codecov tooltipped tooltipped-n\" aria-label=\"Overall coverage\">" + (Math.floor(res['report']['coverage'])) + "%</a>");
      return $('.file-wrap tr:not(.warning):not(.up-tree)').each(function() {
        ...
        if (filepath) {
          ...
          if (!(coverage != null ? coverage.ignored : void 0)) {
            if (coverage >= 0) {
              return $('td:last', this).remove('.codecov').append("<span class=\"sha codecov tooltipped tooltipped-n\" aria-label=\"Coverage\">" + (Math.floor(coverage)) + "%</span>");
            }
          }
        }
      });
      ...
      if ((ref = this.page) === 'commit' || ref === 'compare' || ref === 'pull') {
        if (res['base']) {
          ...
          plus = compare > 0 ? '+' : '-';
          $('.toc-diff-stats').remove('.codecov').append(compare === '0' ? '<span class="codecov">Coverage did not change.</span>' : "<span class=\"codecov\"> Coverage changed <strong>" + plus + compare + "%</strong></span>");
          $('#diffstat').remove('.codecov').append("<span class=\"codecov text-diff-" + (compare > 0 ? 'added' : 'deleted') + " tooltipped tooltipped-s\" aria-label=\"Coverage " + (compare > 0 ? 'increased' : 'decreased') + " " + plus + compare + "%\">" + plus + compare + "%</span>");
        } else {
      ...
      $('#toc li').each(function() {
        ...
        if (!(coverage != null ? coverage.ignored : void 0)) {
          ...
          if (cov >= 0) {
            return $('.diffstat.right', this).remove('.codecov').prepend("<span class=\"codecov\">" + (Math.round(cov)) + "%</span>");
          }
        }
      });
      self = this;
          ...
          if (coverage) {
            ...
            if ((ref2 = self.page) === 'blob' || ref2 === 'blame') {
              ...
              for (i = 0, len = ref3.length; i < len; i++) {
                _ = ref3[i];
                results.push(button.unbind().trigger('click'));
              }
              return results;
            }
          } else {
...
window.Bitbucket = (function(superClass) {
  ...
  Bitbucket.prototype.overlay = function(res) {
    ...
    $('#source-list tr').each(function() {
      ...
      if (!(coverage != null ? coverage.ignored : void 0)) {
        cov = coverage != null ? coverage.coverage : void 0;
        return $('td.size', this).after('<td title="Coverage" class="codecov">' + (cov >= 0 ? (Math.round(cov)) + "%" : '') + "</td>");
      }
    });
    $('section.bb-udiff').each(function() {
      ...
      if (!(coverage != null ? coverage.ignored : void 0)) {
        if (coverage) {
          button = $('.aui-button.codecov', this).attr('title', 'Toggle Codecov').text('Coverage ' + coverage['coverage'].toFixed(0) + '%').unbind().click(self.toggle_diff);
    ...
    return $("#editor-container").each(function() {
      ...
      if (!(coverage != null ? coverage.ignored : void 0)) {
        if (coverage) {
          button = file.find('.aui-button.codecov').attr('title', 'Toggle Codecov').text('Coverage ' + coverage['coverage'].toFixed(0) + '%').unbind().click(self.toggle_coverage);
          ref = coverage['lines'];
          for (ln in ref) {
            cov = ref[ln];
