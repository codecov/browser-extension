...
window.Github = (function(superClass) {
  ...
  Github.prototype.overlay = function(res) {
    ...
    this.log('::overlay');
    self = this;
    $('.codecov:not(.btn)').remove();
    if (this.page === 'tree') {
      replacement = "/" + self.slug + "/blob/" + ($('.file-navigation .repo-root a:first').attr('data-branch')) + "/";
      $('.commit-meta').prepend("<a href=\"" + this.settings.urls[this.urlid] + "/github/" + this.slug + "?ref=" + this.ref + "\" class=\"sha-block codecov tooltipped tooltipped-n\" aria-label=\"Overall coverage\">" + (res['report']['coverage'].toFixed(2)) + "%</a>");
      return $('.file-wrap tr:not(.warning):not(.up-tree)').each(function() {
        ...
        if (filepath) {
          ...
          if (!(coverage != null ? coverage.ignored : void 0)) {
            if (coverage >= 0) {
              return $('td:last', this).append("<span class=\"sha codecov tooltipped tooltipped-n\" aria-label=\"Coverage\">" + (coverage.toFixed(2)) + "%</span>");
            }
          }
        }
      });
      ...
      if ((ref = this.page) === 'commit' || ref === 'compare' || ref === 'pull') {
        if (res['base']) {
          ...
          plus = compare > 0 ? '+' : '-';
          $('.toc-diff-stats').append(compare === '0.00' ? '<span class="codecov">Coverage did not change.</span>' : "<span class=\"codecov\"> Coverage changed <strong>" + plus + compare + "%</strong></span>");
          $('#diffstat').append("<span class=\"codecov text-diff-" + (compare > 0 ? 'added' : 'deleted') + " tooltipped tooltipped-s\" aria-label=\"Coverage " + (compare > 0 ? 'increased' : 'decreased') + " " + plus + compare + "%\">" + plus + compare + "%</span>");
        } else {
          coverage = res['report']['coverage'].toFixed(2);
          if (!(coverage != null ? coverage.ignored : void 0)) {
            $('.toc-diff-stats').append("<span class=\"codecov\"> Coverage <strong>" + coverage + "%</strong></span>");
            $('#diffstat').append("<span class=\"codecov tooltipped tooltipped-s\" aria-label=\"Coverage\">" + coverage + "%</span>");
          }
        }
      }
      $('#toc li').each(function() {
        ...
        if (!(coverage != null ? coverage.ignored : void 0)) {
          ...
          if (cov >= 0) {
            return $('.diffstat.right', this).prepend("<span class=\"codecov\">" + (Math.round(cov)) + "%</span>");
          }
        }
      });
      self = this;
