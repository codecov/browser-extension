    ...
    if (this.page === 'tree') {
      ...
      if ((ref = this.page) === 'commit' || ref === 'compare' || ref === 'pull') {
        if (res['base']) {
          ...
          plus = compare > 0 ? '+' : '-';
          $('.toc-diff-stats').remove('.codecov').append(compare === '0.00' ? '<span class="codecov">Coverage did not change.</span>' : "<span class=\"codecov\"> Coverage changed <strong>" + plus + compare + "%</strong></span>");
